...
void on_solveButton_clicked(GtkWidget *widget, gpointer data) {
    ...
    for (int r = 0; r < m; r++) {
        ...
        double b = entry_to_double(rhs_entry);
        if (b < 0) {
            todos_no_negativos = FALSE;
            break;
        }
    }

    if (!todos_no_negativos) {
        GtkWindow *win = NULL;
        ...
        GtkWidget *dlg = gtk_message_dialog_new(
            ...
            GTK_MESSAGE_WARNING,
            GTK_BUTTONS_OK,
            "Los valores del lado derecho (b) deben ser mayores o iguales a 0.\n"
            "Revise las restricciones e inténtelo nuevamente."
        );
        gtk_window_set_title(GTK_WINDOW(dlg), "Valor no permitido");
